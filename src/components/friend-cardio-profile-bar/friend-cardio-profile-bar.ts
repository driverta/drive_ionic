...
export class FriendCardioProfileBarComponent {
    ...
    queryHistory.once("value").then( snapshot => {
      ...
      snapshot.forEach( childSnapshot => {
        ...
        this.tempRec.forEach( (value, index) => {
          ...
          if (this.loop == snapshot.numChildren()){
            this.tempRec.forEach ((value) => {
            if (value.records > 0){
              this.records._chart.push(value)
            }
          })
          console.log(this.records._chart)
          this.sortRecords();
        } 
      })
    })
  ...
  sortRecords() {
    this.records._chart.sort((a: any, b: any) => {
  ...
  initSvg() {
    ...
    this.x.domain(this.records._chart.map((d) => d.max));
    this.y.domain([0, d3Array.max(this.records._chart, (d) => d.mph)]);
  }
